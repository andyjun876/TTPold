...
public class TestCase {
		...
		try {
            ...
            if(datanum==0){
	            ...
	            while(rs.next()) {
	            		num++;
	            		for(int i=0;i<count;i++){
	            			String getstr = rs.getString(col[i]);
	            			if(getstr!=null&&getstr.indexOf("\"")>0){
	            				getstr = getstr.replace("\"", "\\\"");
	            			}
	            			data = data + "\"" + col[i] + "\":\"" + getstr + "\"";
	            			if(i<count-1){
	                			data = data + ",";
	                		}else {
	                			data = data + "";}
	...
	public String getmodule(String id) throws FileNotFoundException, IOException{
		String message;
		String sql;
		con=new MysqlConnect().getConnection();  
		
		if(id.equals("0")){
			sql=new PropertiesTest().propertiesget("getmodule");
		}else{
			sql=new PropertiesTest().propertiesget("getmodule")+" where a.id='"+id+"';";
		}
		String data = "[{";
		Statement statement;
		try {
